...
def scrap_site (url, comment):
    ...
    # 각각의 content를 찾아와서 db에다가 저장한다 => pymongo
...
def start_server ():
    ...
    def post():
        if 'url' not in request.form or 'comment' not in request.form:
            result = {'status': 'error', 'msg': 'url과 comment를 넣어주세요'}
            return jsonify(result)

        # url과 comment 얻기
        # request 와 requests는 다르다. request는 Flask에 속함
        ...
        # 결과 보내주기
        if result is True:
            result = {'status': 'ok', 'msg': '저장되었습니다.'}
            return jsonify(result)

        result = {'status': 'fail', 'msg': '지원하지 않는 사이트입니다.'}
        return jsonify(result)
